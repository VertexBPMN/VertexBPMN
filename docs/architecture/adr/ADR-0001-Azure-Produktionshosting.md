...
|---|---|---|
| Region | **germanywestcentral** | Datenresidenz DE, niedrige Latenz |
| Resource Group | **vertexbpmnRg** (eine prod-Umgebung) | Alles wird darunter angelegt; Namen mit Suffixen `-api`/`-studio`/`-worker` |
...
- Öffentlich: **Studio** (`studio.vertexbpmn.com`) **und API** (CLI/SDK/Webhooks) — jeweils mit eigenen Rate-Limits und WAF.
- Kein direkter öffentlicher Ingress auf ACA für API/Worker; nur Hinter Front Door.
- **Custom-Domain:** `studio.vertexbpmn.com`.
