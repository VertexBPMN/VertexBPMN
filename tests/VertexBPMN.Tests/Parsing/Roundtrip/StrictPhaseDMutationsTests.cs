...
public class StrictPhaseDMutationsTests
{
    ...
    [Fact]
    public async Task MarkDirtyOnAnyChange_Sets_RoundtripDirty_And_Diagnostic()
    {
  ...
  <bpmn:process id='p1'>
    ...
    [Fact]
    public async Task ApplyAttributeChange_Task_Name_Updates_Model_And_Sets_Dirty()
    {
        const string xml = @"<bpmn:definitions xmlns:bpmn='http://www.omg.org/spec/BPMN/20100524/MODEL'>
  <bpmn:process id='p1'>
    ...
    [Fact]
    public async Task StrictSerializer_Adds_Dirty_Fallback_Diagnostic_When_RoundtripDirty()
    {
        const string xml = @"<bpmn:definitions xmlns:bpmn='http://www.omg.org/spec/BPMN/20100524/MODEL'>
  <bpmn:process id='p1'>
    <bpmn:userTask id='t1' name='Orig'/>
