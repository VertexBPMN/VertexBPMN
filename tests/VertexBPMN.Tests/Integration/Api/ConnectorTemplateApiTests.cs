...
public sealed class ConnectorTemplateApiTests
{
    ...
    public async Task TemplateLifecycle_IsTenantScoped_AndProtectsMutations()
    {
        ...
        var create = await _client.PostAsJsonAsync("/api/connector-templates", Template(tenantId, "HTTP request"));
        Assert.Equal(HttpStatusCode.Created, create.StatusCode);
        var created = await create.Content.ReadFromJsonAsync<ConnectorTemplateMetadata>(JsonOptions);
        ...
        var listed = await _client.GetFromJsonAsync<List<ConnectorTemplateMetadata>>($"/api/connector-templates?tenantId={tenantId}", JsonOptions);
        Assert.Contains(listed!, item => item.Id == created.Id);

        var update = await _client.PutAsJsonAsync($"/api/connector-templates/{created.Id}", Template(tenantId, "HTTP request v2"));
        Assert.Equal(HttpStatusCode.NoContent, update.StatusCode);

        var otherTenant = await _client.GetAsync($"/api/connector-templates/{created.Id}?tenantId=other-{tenantId}");
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await _client.DeleteAsync($"/api/connector-templates/{created.Id}?tenantId={tenantId}")).StatusCode);
    }
