...
internal sealed class MessageFlowParticipantRule : IBpmnSemanticRule
{
    public IEnumerable<ValidationDiagnostic> Evaluate(BpmnModel model, SemanticValidationContext ctx)
    {
        ...
        if (model.Participants != null && model.ProcessDefinitions?.RootElement != null)
        {
            var processIds = model.ProcessDefinitions.RootElement.OfType<Process>().Select(p => p.Id).Where(id => id != null).ToHashSet()!;
            foreach (var part in model.Participants)
            {
                if (part.ProcessRef.Name is string pid && !processIds.Contains(pid))
