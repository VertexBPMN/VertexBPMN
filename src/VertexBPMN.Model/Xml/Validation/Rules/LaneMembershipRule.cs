...
internal sealed class LaneMembershipRule : IBpmnSemanticRule
{
    public IEnumerable<ValidationDiagnostic> Evaluate(BpmnModel model, SemanticValidationContext ctx)
    {
        ...
        foreach (var lane in model.Lanes)
        {
            ...
            foreach (var fn in lane.FlowNodeRefs)
            {
                if (fn.Id is null) continue;
                if (!membership.TryGetValue(fn.Id, out var list))
                    membership[fn.Id] = list = new List<string>();
                list.Add(lane.Id);
                if (!ctx.FlowNodesById.ContainsKey(fn.Id))
                    diagnostics.Add(Error("BPMN160", $"Lane '{lane.Id}' references unknown FlowNode '{fn.Id}'", lane.Id, "Lane"));
            }
        }
