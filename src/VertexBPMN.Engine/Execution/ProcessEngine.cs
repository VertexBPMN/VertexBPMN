...
public class ProcessEngine : IProcessEngine
{
    ...
    /// </summary>
    private string? HandleGateway(BpmnGateway gateway, List<BpmnSequenceFlow> flows, BpmnModel model, List<string> trace)
    {
    ...
    private string? HandleComplexGateway(BpmnGateway gateway, List<BpmnSequenceFlow> flows, BpmnModel model, List<string> trace)
    {
    ...
    private string? HandleEventBasedGateway(BpmnGateway gateway, List<BpmnSequenceFlow> flows, BpmnModel model, List<string> trace)
    {
    ...
    /// </summary>
    private string? HandleSubprocess(BpmnSubprocess subprocess, List<BpmnSequenceFlow> flows, BpmnModel model, List<string> trace)
    {
    ...
    /// </summary>
    private async Task<string?> HandleTaskAsync(BpmnTask task, List<BpmnSequenceFlow> flows, BpmnModel model, List<string> trace, IDecisionService? decisionService)
    {
        ...
        switch (task.Type)
        {
            ...
            default:
                trace.Add($"Task: {task.Id} ({task.Type})");
                break;
        }
    ...
    private async Task HandleBusinessRuleTaskAsync(BpmnTask task, BpmnModel model, List<string> trace, IDecisionService? decisionService)
    {
    ...
    private async Task HandleScriptTaskAsync(BpmnTask task, BpmnModel model, List<string> trace)
    {
    ...
    private async Task HandleServiceTaskAsync(BpmnTask task, BpmnModel model, List<string> trace)
    {
    ...
    {
        return boundaryEvent.EventDefinitionType switch
        {
    ...
    private bool SimulateEventArrival(BpmnEvent targetEvent)
    {
        ...
        _logger.LogInformation("Simulating event arrival for event: {EventId} of type: {EventType}", 
            targetEvent.Id, targetEvent.EventDefinitionType);

        return targetEvent.EventDefinitionType switch
        {
