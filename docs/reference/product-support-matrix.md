...
| Simulation und Simulation Analytics | `partial` | Der Engine-Simulator nutzt den produktiven sicheren BPMN-Parser und den Runtime-Bedingungsauswerter; Exclusive/Inclusive/Parallel/Complex-Splits, Parallel-/dynamische Inclusive-Joins, eingebettete Subprozesse, explizit ausgewählte Event-Gateway-Pfade, deterministische Traces, MaxSteps und hash-gebundene Analytics sind durch `DeterministicSimulationServiceTests` sowie P4-AC-05 belegt. Call Activities, Event-Subprozesse und Multi-Instance-Subprozesse werden weiterhin explizit abgewiesen und verhindern noch Full Support. |
