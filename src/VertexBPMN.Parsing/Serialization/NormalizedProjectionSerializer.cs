...
/// <summary>
/// Phase 8: Deterministic normalization serializer for runtime deployment artifacts.
...
public class NormalizedProjectionSerializer
{
    ...
    private void CollectUsedNamespaces(BpmnModel model, SortedDictionary<string, string> namespaceDeclarations)
    {
        ...
        {
            if (evt.ExtensionAttributes != null)
            {
                foreach (var attr in evt.ExtensionAttributes.Keys)
                {
                    if (attr.Contains(':'))
                    {
                        var prefix = attr.Split(':')[0];
        ...
        {
            if (flow.ExtensionAttributes != null)
            {
                foreach (var attr in flow.ExtensionAttributes.Keys)
                {
                    if (attr.Contains(':'))
                    {
                        var prefix = attr.Split(':')[0];
    ...
    private XElement CreateEventElement(BpmnEvent evt, XNamespace ns, SortedDictionary<string, string> namespaceDeclarations)
    {
        ...
        // Add name if present - Fixed: Get name from ExtensionAttributes
    ...
    private XElement CreateSubprocessElement(BpmnSubprocess subprocess, XNamespace ns, SortedDictionary<string, string> namespaceDeclarations)
    {
        ...
        AddVendorExtensions(element, subprocess.ExtensionAttributes, namespaceDeclarations);
    ...
    private XElement CreateSequenceFlowElement(BpmnSequenceFlow flow, XNamespace ns, SortedDictionary<string, string> namespaceDeclarations)
    {
        ...
        var name = flow.ExtensionAttributes?.TryGetValue("name", out var flowName) == true ? flowName : null;
        if (!string.IsNullOrEmpty(name))
        {
            element.Add(new XAttribute("name", name));
        }
    ...
    private static bool HasVendorExtensions(BpmnModel model)
    {
        return model.Tasks.Any(t => t.Attributes != null && t.Attributes.Count > 0) ||
               model.Events.Any(e => e.ExtensionAttributes != null && e.ExtensionAttributes.Count > 0) ||
               model.SequenceFlows.Any(f => f.ExtensionAttributes != null && f.ExtensionAttributes.Count > 0) ||
               model.RawMetadata?.PriorityAttributeNamespace?.Count > 0;
    }

    // Fixed: Get event name from ExtensionAttributes
    private static string GetEventName(BpmnEvent evt)
    {
        // Check extension attributes for name
        return evt.ExtensionAttributes?.TryGetValue("name", out var name) == true ? name : string.Empty;
    }

    // Fixed: Get attachedToRef from ExtensionAttributes
    private static string GetAttachedToRef(BpmnEvent evt)
    {
        // Check extension attributes for attachedToRef
        if (evt.ExtensionAttributes?.TryGetValue("attachedToRef", out var attachedRef) == true)
        {
            return attachedRef;
        }
