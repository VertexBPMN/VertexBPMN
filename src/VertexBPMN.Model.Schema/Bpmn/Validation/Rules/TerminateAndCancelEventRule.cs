...
internal sealed class TerminateAndCancelEventRule : IBpmnSemanticRule
{
    public IEnumerable<ValidationDiagnostic> Evaluate(BpmnModel model, SemanticValidationContext ctx)
    {
        ...
        foreach (var end in model.Events.OfType<EndEvent>())
        {
            ...
            var terminateDefs = end.EventDefinition.OfType<TerminateEventDefinition>().Count();
            if (terminateDefs > 1)
        ...
        {
            var cancelDefs = boundary.EventDefinition.OfType<CancelEventDefinition>().ToList();
            if (cancelDefs.Count > 0)
            {
                var attachedTo = model.Subprocesses
                    .OfType<Transaction>()
                    .FirstOrDefault(t => t.Id == boundary.AttachedToRef?.Name);
