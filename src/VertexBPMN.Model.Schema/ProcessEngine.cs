...
public class ProcessEngine
{
    ...
    private string? HandleSubprocess(SubProcess subprocess, List<SequenceFlow> flows, BpmnModel model, List<string> trace)
    {
        ...
        else if (subprocess.LoopCharacteristics is Transaction)
        {
            trace.Add($"TransactionSubprocess: {subprocess.Id}");
            var compensationEvent = model.Events.FirstOrDefault(e =>
                e is BoundaryEvent b &&
                b.AttachedToRef.Name == subprocess.Id && subprocess.IsForCompensation );
            HandleCompensationEvent(compensationEvent, subprocess.Id, trace);
        ...
        var isMultiInstance = subprocess.LoopCharacteristics is MultiInstanceLoopCharacteristics;
        if (isMultiInstance)
        {
    ...
    private async Task HandleServiceTaskAsync(Bpmn.Task task, BpmnModel model, List<string> trace)
    {
        trace.Add($"ServiceTask: {task.Id} ({task.Name})");
    ...
    private string? HandleUserTask(Bpmn.Task task, List<string> trace)
    {
        trace.Add($"UserTask: {task.Id} ({task.Name})");

        // Log task attributes
        if (task.AnyAttribute != null)
        {
            foreach (var attribute in task.AnyAttribute)
            {
                trace.Add($"Attribute: {attribute.Name} = {attribute.Value}");
            }
        }

        // Safely extract assignee and dueDate from AnyAttribute (which is List<XmlAttribute>)
        string assignedTo = "Unassigned";
        string? dueDate = null;
        if (task.AnyAttribute != null)
        {
            var assigneeAttr = task.AnyAttribute.FirstOrDefault(a => a.Name == "assignee");
            if (assigneeAttr != null)
                assignedTo = assigneeAttr.Value;

            var dueDateAttr = task.AnyAttribute.FirstOrDefault(a => a.Name == "dueDate");
            if (dueDateAttr != null)
                dueDate = dueDateAttr.Value;
        }
    ...
    //private async Task HandleZeebeAttributesAsync(VertexBPMN.Domain.Model.Bpmn.Task task, BpmnModel model, List<string> trace)
    //{
    ...
    private async Task HandleUnregisteredServiceTaskAsync(Bpmn.Task task, IDictionary<string, object>? processVariables, List<string> trace)
    {
        ...
        var resultVariable = $"{task.Id}_Result"; // No Attributes, so use default naming
    ...
    private void HandleMultiInstanceSubprocess(SubProcess subprocess, BpmnModel model, List<string> trace)
    {
        ...
        // FIX: SubProcess does not have LoopCardinality property.
        // Try to get loop cardinality from LoopCharacteristics if available.
        int totalInstances = 3; // Default value

        if (subprocess.LoopCharacteristics is MultiInstanceLoopCharacteristics miLoop && int.TryParse(miLoop.LoopCardinality.Text.FirstOrDefault(), out int parsedCardinality) && parsedCardinality >= 1)
        {
            totalInstances = parsedCardinality;
        }
    ...
    private void HandleEventSubprocess(SubProcess subprocess, BpmnModel model, List<string> trace)
    {
        ...
        var subprocessStartEvents = model.Events.Where(e =>
            e is StartEvent && IsWithinSubprocess(e.Id, subprocess.Id, model)).ToList();

        foreach (StartEvent startEvent in subprocessStartEvents)
        {
            trace.Add($"EventSubprocessStart: {startEvent.Id}");

            if (startEvent.EventDefinition.Any())
            {
                trace.Add($"EventType: {startEvent.EventDefinition.FirstOrDefault()}");

                switch (startEvent.Id)
                {
                    case  "message":
        ...
        foreach (EndEvent endEvent in subprocessEndEvents)
        {
    ...
    {
        return boundaryEvent.EventDefinition.First().Id switch
        {
    ...
    private SequenceFlow? SelectEventBasedFlow(Gateway gateway, List<SequenceFlow> flows, BpmnModel model, List<string> trace)
    {
        ...
        foreach (var flow in flows)
        {
            ...
            if (targetEvent != null)
            {
                trace.Add($"EventTarget: {targetEvent.GetType()} {targetEvent.Id}");
    ...
    private bool SimulateEventArrival(Event targetEvent)
    {
        ...
        _logger.LogInformation("Simulating event arrival for event: {EventId} of type: {EventType}",
            targetEvent.Id, targetEvent.GetType());

        return targetEvent.Name switch
        {
            "message" => SimulateMessageEvent((CatchEvent)targetEvent),
            "timer" => SimulateTimerEvent((CatchEvent)targetEvent),
            "signal" => SimulateSignalEvent((CatchEvent)targetEvent),
            "error" => SimulateErrorEvent((CatchEvent)targetEvent),
            "condition" => SimulateConditionEvent((CatchEvent)targetEvent),
            _ => false
        };
    }
    ...
    private bool SimulateMessageEvent(CatchEvent targetEvent)
    {
        ...
        var definition = targetEvent.EventDefinition.FirstOrDefault(d => d is MessageEventDefinition) as MessageEventDefinition;
    ...
    private bool SimulateSignalEvent(CatchEvent targetEvent)
    {
        ...
        var definition = targetEvent.EventDefinition.FirstOrDefault(d => d is SignalEventDefinition) as SignalEventDefinition;
    ...
    private bool SimulateErrorEvent(CatchEvent targetEvent)
    {
        ...
        var definition = targetEvent.EventDefinition.FirstOrDefault(d => d is ErrorEventDefinition) as ErrorEventDefinition;
    ...
    private bool SimulateConditionEvent(CatchEvent targetEvent)
    {
        ...
        var definition = targetEvent.EventDefinition.FirstOrDefault(d => d is ConditionalEventDefinition) as ConditionalEventDefinition;
