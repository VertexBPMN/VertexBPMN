...
public sealed partial class FullConformanceProcessEngine : IProcessEngine
{
    private readonly ILogger<FullConformanceProcessEngine> _logger;
    ...
    public FullConformanceProcessEngine() : this(NullLogger<FullConformanceProcessEngine>.Instance,
        NullServiceTaskRegistry.Instance)
    {
    }

    public FullConformanceProcessEngine(ILogger<FullConformanceProcessEngine> logger,
    ...
    #region IProcessEngine Unsupported Features

    public Task RegisterBpmnModelAsync(string processId, string bpmnXml, CancellationToken cancellationToken = default)
    {
        return Task.FromException(
            new NotSupportedException("Model registry not supported in full-conformance reference engine"));
    }
    ...
    {
        return Task.FromException(
            new NotSupportedException("DMN registration not supported (provide IDecisionService externally)"));
    }
    ...
    {
        return Task.FromException<List<string>>(new NotSupportedException("Process registry not implemented"));
    }
    ...
    private void HandleGatewayForToken(BpmnGateway g, BpmnModel model, List<string> trace, string tokenId, string? parentTxn)
    {
        ...
        switch (g.Type)
        {
            case "exclusiveGateway":
            {
                ...
                var selected = SelectExclusiveGatewayFlow(g.Id, outs, vars, trace);
    ...
    private void HandleTaskForToken(BpmnTask task, BpmnModel model, IDecisionService? decisionService, List<string> trace, string tokenId, string? parentTxn)
    {
        ...
        else if (task.Type == "businessRuleTask" && decisionService != null)
        {
            try
            {
                var decisionKey =
...
partial class FullConformanceProcessEngine
{
        ...
        IDictionary<string, object> vars, List<string> trace)
    {
        ...
        // Order: Priority (ascending) -> original order
        var ordered = flows
            .Select(f => (Flow: f, Priority: GetPriority(f) ?? int.MaxValue))
            .OrderBy(t => t.Priority)
            .Select(t => t.Flow)
            .ToList();

        BpmnSequenceFlow? defaultFlow = ordered.FirstOrDefault(IsDefaultFlow);

        // Evaluate each conditional flow
        foreach (var flow in ordered)
        {
            var cond = GetConditionExpression(flow);
            if (string.IsNullOrWhiteSpace(cond)) continue;
            if (EvaluateSimpleCondition(cond!, vars))
            {
                trace.Add($"ExclusiveConditionMatched: {flow.Id}");
                return flow;
            }
        }
        ...
        if (defaultFlow != null)
        {
            trace.Add($"ExclusiveDefaultTaken: {defaultFlow.Id}");
            return defaultFlow;
        }

        var fallback = ordered.FirstOrDefault();
        if (fallback != null)
            trace.Add($"ExclusiveFallbackFirst: {fallback.Id}");
        return fallback;
    }

    private static bool EvaluateSimpleCondition(string rawExpr, IDictionary<string, object> vars)
    {
        if (string.IsNullOrWhiteSpace(rawExpr)) return false;
        var expr = rawExpr.Trim();

        // Strip ${ ... } if FEEL / expression wrapper
        if (expr.StartsWith("${") && expr.EndsWith("}"))
            expr = expr[2..^1].Trim();

        // Normalize single '=' (e.g., 'A = "B"') to '=='
        ...
        if (eqIdx > 0)
        {
            var left = expr[..eqIdx].Trim();
            var right = TrimQuotes(expr[(eqIdx + 2)..].Trim());
            if (!vars.TryGetValue(left, out var val)) return false;
            return string.Equals(val?.ToString(), right, StringComparison.OrdinalIgnoreCase);
        }

        // Inequality
        var neIdx = expr.IndexOf("!=", StringComparison.Ordinal);
        if (neIdx > 0)
        {
            var left = expr[..neIdx].Trim();
            var right = TrimQuotes(expr[(neIdx + 2)..].Trim());
            if (!vars.TryGetValue(left, out var val)) return false;
            return !string.Equals(val?.ToString(), right, StringComparison.OrdinalIgnoreCase);
        }

        // Presence check
        return vars.ContainsKey(expr);
    }

    private static bool IsDefaultFlow(BpmnSequenceFlow flow)
    {
        try
        {
            var p = flow.GetType().GetProperty("IsDefault");
            if (p?.GetValue(flow) is bool b) return b;
        }
        catch { }
        return false;
    }

    private static int? GetPriority(BpmnSequenceFlow flow)
    {
        try
        {
            var p = flow.GetType().GetProperty("Priority");
            var v = p?.GetValue(flow);
