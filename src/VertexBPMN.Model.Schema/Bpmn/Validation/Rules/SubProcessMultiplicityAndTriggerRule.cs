...
internal sealed class SubProcessMultiplicityAndTriggerRule : IBpmnSemanticRule
{
    public IEnumerable<ValidationDiagnostic> Evaluate(BpmnModel model, SemanticValidationContext ctx)
    {
        ...
        // Replace sp.IsMultiInstance with a check for LoopCharacteristics indicating multi-instance
        foreach (var sp in model.Subprocesses)
        {
            if (sp.Id is null) continue;
            // Multi-instance heuristic: warn if LoopCharacteristics is MultiInstanceLoopCharacteristics and no internal elements >1
            if (sp.LoopCharacteristics is MultiInstanceLoopCharacteristics && (sp.FlowElement.Count == 0))
            ...
            if (sp is Transaction)
            {
                var boundaries = model.Events.OfType<BoundaryEvent>().Where(b => b.AttachedToRef?.Name == sp.Id).ToList();
                var hasCancel = boundaries.Any(b => b.EventDefinition.OfType<CancelEventDefinition>().Any());
                if (!hasCancel)
                    diagnostics.Add(Warning("BPMN133", $"Transaction SubProcess '{sp.Id}' without cancel boundary event", sp.Id, "SubProcess"));
            }
        }
