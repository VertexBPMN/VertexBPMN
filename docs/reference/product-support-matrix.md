...
| Externe Brokerzustellung der Outbox | `supported` | Der persistente Publisher least Nachrichten atomar und replika-sicher und liefert mit stabilen Message-IDs, Retry-/Dead-Letter-Semantik und Broker-Readiness an RabbitMQ oder Kafka. Echte RabbitMQ-/PostgreSQL-Akzeptanztests werden lokal mit expliziten Verbindungsdaten qualifiziert; sie gehören nicht zum schnellen CI. |
...
| Build und Release | `supported` | Pull Requests und `master` werden durch Restore, einen Release-Build und das zentrale Testprojekt blockiert. `v*`-Tags paketieren SDK und CLI mit der Tag-Version und veröffentlichen exakt diese Build-Artefakte über NuGet OIDC. Browser-, Benchmark-, Audit-, Coverage-, OpenAPI-/Conformance-, Container-, SBOM- und externe RabbitMQ-/PostgreSQL-Prüfungen bleiben gezielt ausführbar, blockieren den schnellen Standardworkflow jedoch nicht. |

## Verbindliche Acceptance-Fälle

Die Runtime-Verträge verwenden einen echten `WebApplicationFactory`-Host und relationale Persistenz. Die fachlichen Assertions prüfen End-/Wait-State und dauerhafte Runtime-Datensätze. Die vier `P3-EXT`-Verträge benötigen echte RabbitMQ- und PostgreSQL-Dienste. Sie sind aus dem schnellen CI ausgeschlossen und werden lokal bei gesetzten Verbindungsdaten ausgeführt. `scripts/test-production-readiness.ps1` verlangt ihre Ausführung ohne Skips; ein vollständiger aktueller Lauf ist Voraussetzung für den entsprechenden Abnahmenachweis.
...
Die historische Full-Product-Support-Qualifizierung umfasst 51 konkrete Testfälle: 47 Fälle im Hauptgate sowie vier externe RabbitMQ-/PostgreSQL-Verträge. Darin enthalten sind 19 persistente BPMN-Core-Verträge, Runtime-Outbox-Analytics, fünf Live-Migrations-, vier Compensation-, acht FEEL-/DRD- und ein Studio-Runtime-Vertrag. Sechs zusätzliche Phase-2-Verträge tragen `Category=Phase2Acceptance`. Für aktuelle Releases sind die tatsächlichen Laufzahlen und Ergebnisse des lokalen Abnahmeberichts maßgeblich; historische grüne Tabellen ersetzen diesen Nachweis nicht.
