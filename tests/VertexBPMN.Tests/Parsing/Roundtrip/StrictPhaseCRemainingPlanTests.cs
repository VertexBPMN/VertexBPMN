...
public class StrictPhaseCRemainingPlanTests
{
    ...
    // A. RawDiRoot unver�ndert ausgeben
    [Fact]
    public async Task Strict_Emits_RawDiRoot_Unchanged() // RED
    {
  ...
  <bpmndi:BPMNDiagram id='D1'>
    <bpmndi:BPMNPlane bpmnElement='p1'>
      <bpmndi:BPMNShape id='shape_s1' bpmnElement='s1'>
        ...
        var model = await Strict().ParseAsync(xml, TestContext.Current.CancellationToken);
    ...
    // B. LaneSet Struktur unver�ndert (laneSet->lane->flowNodeRef Reihenfolge)
    [Fact]
    public async Task Strict_Emits_LaneSet_Structure_Unchanged() // RED
    {
  ...
  <bpmn:process id='p1'>
    ...
    // C. NamespaceContext Reihenfolge exakt beibehalten + kein zus�tzlicher bpmn Prefix, wenn urspr�nglich nur Default-NS
    [Fact]
    public async Task Strict_Preserves_NamespaceContext_Order_And_No_Extra_BpmnPrefix_When_DefaultOnly() // RED
    {
  ...
  <process id='p1'/>
    ...
    // D. Fallback Diagnostics wenn Raw-Bl�cke fehlen (extensions Beispiel)
    [Fact]
    public async Task Fallback_Diagnostics_When_RawExtensions_Missing() // RED
    {
  ...
  <bpmn:process id='p1'>
    ...
    [Fact]
    public async Task Strict_RawExtensions_Immutable_Snapshot() // RED (current behavior mutable)
    {
        const string xml = @"<bpmn:definitions xmlns:bpmn='http://www.omg.org/spec/BPMN/20100524/MODEL' xmlns:v='http://vendor/x'>
  <bpmn:process id='p1'>
