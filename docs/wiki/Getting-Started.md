...
API und Studio können gemeinsam gestartet werden:

```powershell
dotnet run --project src/VertexBPMN.Cli -- dashboard
```

Der CLI-Workflow verwendet standardmäßig:
...
Prüfe den Health-Endpunkt:

```powershell
curl http://localhost:51870/api/Health
```

Die tatsächliche URL kann durch Launch-Settings abweichen. Swagger und die CLI-Ausgabe sind maßgeblich.
